...
import jp.techacademy.youichi.okami.apiapp.databinding.RecyclerFavoriteBinding
...
class FavoriteItemViewHolder(private val binding: RecyclerFavoriteBinding) :
    ...
    fun bind(favoriteShop: FavoriteShop, position: Int, adapter: FavoriteAdapter) {
        binding.rootView.apply {
            ...
            // クリック時のイベントリスナーを割り当て
            setOnClickListener {
                adapter.onClickItem?.invoke(favoriteShop.url,favoriteShop.id,favoriteShop.name,"aaa",favoriteShop.imageUrl)
            }
        }
